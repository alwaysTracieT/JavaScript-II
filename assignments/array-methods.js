...
// // Problem 1 DONE
// The heats are being divided up alphabetically by company name. I used .sort() to put the runners in the right order
let companyGroups = [];
companyGroups = runners.sort(function(a, b){
...
// // Problem 2 DONE
// We want to send thank you cards to people who donated over $100. I'm using .filter() to find their email addresses

let bigDonations = [];
bigDonations = runners.filter(function(amount){
    if (amount.donation > 100) {
        return true;
    };
});
...
// // Problem 3 DONE
//Since we only need the emails for the thank you notes, I took away the other information with .forEach()

let emailList = [];
bigDonations.forEach(function(emails){
    return emailList.push(`${emails.email}`);
});
